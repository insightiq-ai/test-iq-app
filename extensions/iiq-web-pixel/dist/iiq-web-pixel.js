(() => {
  ...
  var init_register = __esm({
    "node_modules/@shopify/web-pixels-extension/build/esm/register.mjs"() {
      init_globals();
      register = (extend2) => shopify.extend(EXTENSION_POINT, extend2);
    }
  });
  ...
  var require_src = __commonJS({
    "extensions/iiq-web-pixel/src/index.js"(exports) {
      ...
      var sendTracking = (_0) => __async(exports, [_0], function* ({ event, fingerprint, demographics_info, device_metadata, utm_params, windowObj, documentObj, customerId }) {
        console.log("Event Name", event.name, fingerprint, demographics_info, device_metadata, utm_params);
        const currentTimestampMilliseconds = (/* @__PURE__ */ new Date()).getTime();
        const currentTimestampSeconds = Math.floor(currentTimestampMilliseconds / 1e3);
        ...
        try {
          ...
          const payload = {
            fingerprint,
            user_session_id: event == null ? void 0 : event.clientId,
            demographics_info,
            device_metadata,
            utm_params,
            events: [
          ...
          const response = yield axios_default.post(url.href, payload);
      ...
      register((_0) => __async(exports, [_0], function* ({ analytics, browser, settings, init }) {
        analytics.subscribe("all_events", (event) => __async(exports, null, function* () {
          var _a, _b;
          const fingerprint = yield browser.localStorage.getItem("fingerprint-insightiq");
          let demographics_info = yield browser.localStorage.getItem("demographics-info-insightiq");
          let device_metadata = yield browser.localStorage.getItem("device-metadata-insightiq");
          let utm_params = yield browser.localStorage.getItem("utm-params-insightiq");
          const windowObj = event.context.window.location;
          const documentObj = event.context.document;
          const customerId = (_b = (_a = init.data) == null ? void 0 : _a.customer) == null ? void 0 : _b.id;
          demographics_info = JSON.parse(demographics_info);
          utm_params = JSON.parse(utm_params);
          device_metadata = JSON.parse(device_metadata);
          if (fingerprint && demographics_info && device_metadata && utm_params)
